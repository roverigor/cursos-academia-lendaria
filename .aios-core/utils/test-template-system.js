...
class TestTemplateSystem {
  ...
  async getTemplate(componentType, testType, framework = 'jest', options = {}) {
    const templateKey = `${framework}-${componentType}-${testType}`;
    ...
    if (!template) {
      template = await this.generateTemplate(componentType, testType, framework, options);
    }
  ...
  async generateTestContent(component, testFile, config) {
    ...
    return await this.renderTemplate(template, templateData);
  }
   ...
   */
  async generateTemplate(componentType, testType, framework, options = {}) {
    const baseTemplate = this.getBaseTemplate(componentType, testType, framework);
    const frameworkConfig = this.frameworks[framework];

    const template = {
      name: `${framework}-${componentType}-${testType}`,
      type: testType,
      framework: framework,
      componentType: componentType,
      imports: this.generateImports(componentType, testType, framework, options),
      setup: this.generateSetup(componentType, testType, framework, options),
      testCases: this.generateTestCaseTemplates(componentType, testType, options),
      teardown: this.generateTeardown(componentType, testType, framework, options),
      mocks: this.generateMockTemplates(componentType, testType, options),
      utilities: this.generateUtilityTemplates(componentType, testType, framework, options),
      metadata: {
...
{{metadata}}`
    ...
    return {
      ...
      util: {
        ...
        setup: {
          ...
          beforeEach: [
            "UtilClass = require('../../../aios-core/utils/{{component.name}}');",
   ...
   */
  generateImports(componentType, testType, framework, options) {
    const frameworkConfig = this.frameworks[framework];
   ...
   */
  generateSetup(componentType, testType, framework, options) {
    const componentTemplate = this.componentTemplates[componentType];
    const setup = {
      variables: [],
      beforeAll: [],
   ...
   */
  generateTeardown(componentType, testType, framework, options) {
    const teardown = {
      afterEach: [],
      afterAll: []
    };
   ...
   */
  generateUtilityTemplates(componentType, testType, framework, options) {
    const utilities = [];
...
// Framework: ${data.config.framework}`;
  ...
  getAvailableTemplates() {
    ...
    for (const [key, template] of this.templateCache.entries()) {
      ...
      templates[framework][componentType].push(testType);
    }
