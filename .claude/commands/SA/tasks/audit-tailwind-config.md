...
- tailwind-merge checker: `eslint --rule "tailwindcss/no-contradicting-classname: error"`

## Notes

- Encourage automated linting (ESLint + prettier-plugin-tailwindcss) post-audit
- Document class naming conventions (order: layout → dimension → spacing → typography → color → effect)
