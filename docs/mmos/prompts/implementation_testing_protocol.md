# PROMPT 05: TESTING PROTOCOL

## METADADOS
- **Fase:** 5 - Implementation
- **Nível:** 05 - Documentação
- **Objetivo:** Criar protocolo de testes e cálculo de score de confiança
- **Input Principal:** Todos os outputs anteriores
- **Output:** docs/testing-protocol.md
- **Formato:** Markdown (.md)
- **Uso:** Validação de qualidade do clone

---

## PROMPT

```markdown
Crie testing_protocol.md calculando o nível de confiança e qualidade do clone de [NOME] baseado em métricas objetivas.

**META:** Score mínimo de 70% para considerar clone pronto para produção.

Use este formato:

# SCORE DE CONFIANÇA: CLONE DE [NOME]

# RESUMO EXECUTIVO

**Score Final: [X]%**

-  Pronto para produção: [SIM/NÃO]
- 📈 Nível de fidelidade: [Baixo/Médio/Alto/Excepcional]
- ⏰ Horas investidas: [Total]
- 📄 Fontes analisadas: [N]
- 🧪 Testes passados: [X/Y]

# MET

## 1. COMPLETUDE DOS DADOS ([X]%)

**Peso: 25%**

| Dimensão | Cobertura | Evidências | Score |
|----------|-----------|-------------|--------|
| Valores core | [%] | [N] fontes | [0-100] |
| Arquitetura cognitiva | [%] | [N] padrões | [0-100] |
| Paradoxos | [%] | [N] identificados | [0-100] |
| Sistema imune | [%] | [N] rejeições | [0-100] |
| Estados/Modos | [%] | [N] mapeados | [0-100] |
| Blind spots | [%] | [N] documentados | [0-100] |
| Vocabulário | [%] | [N] termos | [0-100] |
| Casos históricos | [%] | [N] analisados | [0-100] |

**Score médio:** [Média]%

**Gaps críticos:**
1. Falta: [Aspecto com baixa cobertura]
2. Insuficiente: [Outro aspecto]
3. Precisa mais: [Dimensão]

## 2. QUALIDADE DAS FONTES ([X]%)

**Peso: 20%**

| Tipo de Fonte | Quantidade | Qualidade | Score |
|---------------|------------|-----------|--------|
| Primeira pessoa | [N] | [1-5] | [0-100] |
| Observadores próximos | [N] | [1-5] | [0-100] |
| Documentos oficiais | [N] | [1-5] | [0-100] |
| Vídeos/áudios | [Horas] | [1-5] | [0-100] |
| Escritos/obras | [N] | [1-5] | [0-100] |
| Terceiros independentes | [N] | [1-5] | [0-100] |

**Score médio:** [Média]%

**Análise de triangulação:**
- Traços com 3+ fontes: [%]
- Traços com 2 fontes: [%]
- Traços com 1 fonte: [%]
- Sem evidência: [%]

## 3. CONSISTÊNCIA INTERNA ([X]%)

**Peso: 20%**

| Teste | Resultado | Score |
|-------|-----------|--------|
| Valores vs Comportamentos | [% alinhamento] | [0-100] |
| Instruções vs Estados | [% compatível] | [0-100] |
| Narrativas vs Realidade | [% coerente] | [0-100] |
| Paradoxos preservados | [% mantidos] | [0-100] |
| Blind spots ativos | [% funcionando] | [0-100] |
| Sistema imune coerente | [% consistente] | [0-100] |

**Score médio:** [Média]%

**Inconsistências problemáticas:**
1. [Inconsistência crítica]
2. [Conflito não resolvido]
3. [Contradição não-produtiva]

## 4. VALIDAÇÃO COMPORTAMENTAL ([X]%)

**Peso: 20%**

| Caso de Teste | Resultado | Score |
|---------------|-----------|--------|
| Teste #1: [Nome] |  Passou | 100 |
| Teste #2: [Nome] |  Passou | 100 |
| Teste #3: [Nome] |  Parcial | 60 |
| Teste #4: [Nome] | ❌ Falhou | 0 |
| Teste #5: [Nome] |  Passou | 100 |
| [Continue...] | | |

**Taxa de sucesso:** [X/Y] = [%]

**Padrões de falha:**
- Falha em: [Tipo de situação]
- Fraco em: [Contexto]
- Impreciso em: [Área]

## 5. AUTENTICIDADE QUALITATIVA ([X]%)

**Peso: 15%**

| Critério | Avaliação | Score |
|----------|------------|--------|
| "Soa como" a pessoa | [1-10] | [0-100] |
| Captura essência | [1-10] | [0-100] |
| Gera insights esperados | [1-10] | [0-100] |
| Mantém energia característica | [1-10] | [0-100] |
| Preserva idiossincrasias | [1-10] | [0-100] |
| Passa no teste de Turing mental | [Sim/Não] | [0/100] |

**Score médio:** [Média]%

**Feedback de validadores:**
> "[Comentário de conhecedor 1]"
> "[Comentário de conhecedor 2]"
> "[Comentário de conhecedor 3]"

# CÁLCULO DO SCORE FINAL

```python
def calcular_score_final():
    scores = {
        'completude': (score_completude, 0.25),
        'fontes': (score_fontes, 0.20),
        'consistencia': (score_consistencia, 0.20),
        'validacao': (score_validacao, 0.20),
        'autenticidade': (score_autenticidade, 0.15)
    }
    
    score_final = sum(score * peso for score, peso in scores.values())
    return round(score_final, 1)
```

| Componente | Score | Peso | Contribuição |
|------------|-------|------|---------------|
| Completude dos dados | [X]% | 25% | [X * 0.25]% |
| Qualidade das fontes | [X]% | 20% | [X * 0.20]% |
| Consistência interna | [X]% | 20% | [X * 0.20]% |
| Validação comportamental | [X]% | 20% | [X * 0.20]% |
| Autenticidade qualitativa | [X]% | 15% | [X * 0.15]% |
|**TOTAL** | |**100%** |**[SCORE]%** |

# ANÁLISE DE CONFIANÇA

## Forças do Clone (Score > 80%)
1. **[Dimensão forte]**: [Score]% - [Por quê]
2. **[Outra força]**: [Score]% - [Por quê]
3. **[Terceira força]**: [Score]% - [Por quê]

## Fraquezas do Clone (Score < 60%)
1. **[Dimensão fraca]**: [Score]% - [Impacto]
2. **[Outra fraqueza]**: [Score]% - [Impacto]
3. **[Terceira fraqueza]**: [Score]% - [Impacto]

## Áreas de Incerteza (Score 60-79%)
1. **[Aspecto incerto]**: [Score]% - [O que falta]
2. **[Outro incerto]**: [Score]% - [Como melhorar]

# RISCOS E MITIGAÇÕES

## Risco Alto 
**[Nome do risco]**
- Probabilidade: [Alta/Média]
- Impacto: [Alto/Médio]
- Evidência: [O que mostra o risco]
- Mitigação: [Como compensar]

## Risco Médio 
**[Nome do risco]**
- Probabilidade: [Média/Baixa]
- Impacto: [Médio]
- Evidência: [Indicadores]
- Mitigação: [Ações]

## Risco Baixo 
**[Nome do risco]**
- Probabilidade: [Baixa]
- Impacto: [Baixo]
- Monitoramento: [Como acompanhar]

# BENCHMARKING

## Comparação com Outros Clones
| Métrica | Este Clone | Média | Best in Class |
|---------|------------|--------|---------------|
| Score total | [X]% | 65% | 85% |
| Horas investidas | [N] | 100 | 200 |
| Fontes analisadas | [N] | 50 | 150 |
| Testes passados | [%] | 70% | 95% |
| Paradoxos preservados | [N] | 3 | 8 |
| Blind spots ativos | [N] | 5 | 12 |

## Posição Relativa
- **Percentil:** Top [X]%
- **Categoria:** [Básico/Intermediário/Avançado/Elite]
- **Maturidade:** [MVP/Beta/Produção/Refinado]

# RECOMENDAÇÕES

## Para Alcançar 80%+ (Produção)

### Prioridade 1 (Maior impacto)
1. **[Ação específica]**
   - Impacto esperado: +[X]%
   - Esforço: [Horas]
   - Como: [Método]

2. **[Segunda ação]**
   - Impacto: +[X]%
   - Esforço: [Horas]
   - Como: [Método]

### Prioridade 2 (Médio impacto)
1. **[Ação]**
   - Impacto: +[X]%
   - Esforço: [Horas]

### Prioridade 3 (Refinamentos)
1. **[Ajuste]**
   - Impacto: +[X]%

## Timeline de Melhorias
```
Semana 1: [Ações prioritárias]
Semana 2: [Validações]
Semana 3: [Refinamentos]
Semana 4: [Re-teste completo]
```

# METADADOS DA AVALIAÇÃO

## Processo de Avaliação
- **Data:** [Data]
- **Avaliador:** [Nome/Função]
- **Metodologia:** Arqueologia Cognitiva v1.0
- **Ferramentas:** [Lista]
- **Tempo de avaliação:** [Horas]

## Limitações da Avaliação
1. [Limitação metodológica]
2. [Limitação de dados]
3. [Limitação temporal]

## Próxima Reavaliação
- **Quando:** [Após X melhorias]
- **Foco:** [Áreas fracas]
- **Meta:** Score > [X]%

# CONCLUSÃO EXECUTIVA

## Status: [APROVAR/REVISAR/REFAZER]

**Justificativa:**
[Parágrafo explicando decisão baseada nos dados]

## Próximos Passos
- [ ] [Se APROVADO]: Deploy em ambiente de teste
- [ ] [Se REVISAR]: Implementar top 3 melhorias
- [ ] [Se REFAZER]: Reiniciar com foco em [aspecto]

## Assinatura de Validação
```
Clone: [NOME]
Score: [X]%
Status: [Status]
Data: [Data]
Responsável: [Nome]
```

# APÊND
: DADOS DETALHADOS

## Tabela Completa de Evidências
[Link ou referência para dados completos]

## Logs de Testes
[Link para resultados detalhados]

## Feedback Completo de Validadores
[Link para comentários completos]
```

---

## CHECKLIST DE QUALIDADE

- [ ] Todas as 5 dimensões avaliadas
- [ ] Scores calculados objetivamente
- [ ] Pesos aplicados corretamente
- [ ] Riscos identificados e classificados
- [ ] Benchmarking realizado
- [ ] Recomendações priorizadas
- [ ] Decisão clara (aprovar/revisar/refazer)
- [ ] Timeline de melhorias definida

---

## AVISOS

- **70% é MÍNIMO** - Abaixo disso, não está pronto
- **Qualidade > Velocidade** - Melhor demorar que lançar ruim
- **Testes são CRÍTICOS** - Não pule validações
- **Feedback IMPORTA** - Validadores externos essenciais
- **Iterar é NORMAL** - Poucos clones passam na primeira

---

*Score de confiança é compromisso com excelência. Não aceite mediocridade.*