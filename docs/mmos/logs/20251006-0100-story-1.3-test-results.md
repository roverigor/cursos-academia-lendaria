...
```yaml
rollback_steps:
  1. restore_sources: cp -r outputs/minds/BACKUP_elon_musk_20251006/sources outputs/minds/elon_musk/
  2. cleanup_temp_files: find outputs/minds/elon_musk -name "*brownfield*" -type f
...
**Problem**: Code assumed `minds/{name}` but actual structure is `outputs/minds/{name}`
...
### Issue 3: Backup Strategy (Post-test Refactoring)
**Problem**: Manual BACKUP_* directories are redundant with git versioning
**Fix**: Refactored to use git:
- workflow_executor.py: Use `git rev-parse HEAD` for snapshot
- rollback_manager.py: Use `git reset --hard` for rollback
- plan_generator.py: Updated checklist to use git commit
- Removed all BACKUP_* creation logic
