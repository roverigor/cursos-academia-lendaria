...
CREATE TABLE IF NOT EXISTS minds (
  ...
  -- (Sem campos de tempo agregados: serão derivados via views)
...
CREATE TABLE IF NOT EXISTS sources (
  ...
  title TEXT NOT NULL,
  type TEXT NOT NULL,                      -- 'book','article','podcast','interview',...
  platform TEXT,                           -- 'YouTube','X','Medium'...
  author TEXT,
...
CREATE TABLE IF NOT EXISTS fragments (
  ...
  -- relevância 0..10 + derivado 0.0..1.0
...
DO $$
BEGIN
  -- Se já existe 'tsv' sem ser gerada, recria como coluna gerada (idempotente)
  IF EXISTS (
...
-- Regras:
--   1) scope_type='mind'  → mind_id = scope_id
--   2) scope_type='source'→ mind_id = sources.mind_id
...
-- (D) tokens/custo por fragmento (join direto na execução de origem)
...
-- (E) Agregado por lote/modelo (custo/tokens; duração está em v_batch_durations)
...
-- (F) Status do pipeline por mente
...
-- (G) Heurística de “alta qualidade”
...
-- (H) Painel executivo por mente (usa tempos/custos derivados)
