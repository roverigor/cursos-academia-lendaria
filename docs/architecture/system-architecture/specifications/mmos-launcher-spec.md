...
**Purpose:** CLI tool que mapeia automaticamente prompts MMOS → agentes AIOS, injeta contexto relevante e registra execuções com zero fricção manual.

**Success Criteria MVP:**
- ✅ Reduzir tempo de preparação por prompt em ≥30% (baseline: ~5min manual)
- ✅ 100% rastreabilidade (timestamp, agente, prompt, output path)
- ✅ Zero alteração na estrutura ACS V3.0 existente
- ✅ Funcionar em 1 comando para qualquer dos 59 prompts

**Scope MVP:**
- ✅ CLI Python simples (boring tech)
- ✅ File-based (YAML config + Markdown logs)
- ✅ Read-only sobre estrutura existente (não modifica minds/)
- ❌ NÃO é automation engine (permanece conversacional)
- ❌ NÃO executa prompts automaticamente (usuário ainda opera agentes AIOS)

---

## 1. Functional Requirements (MVP)

### FR1: Prompt → Agent Mapping
- Dado `--prompt` ID, carregar definição de `docs/mmos/prompts.yaml`
...
- Carregar contexto relevante do mind:
  ...
  - `sources/sources_master.yaml` (fontes disponíveis)
...
- Substituir placeholders: `{mind}` → mind name, `{timestamp}` → YYYYMMDD-HHMM
- Validar que diretório pai existe (criar se necessário)

### FR4: Execution Logging
- Registrar invocação em `docs/mmos/logs/launcher-history.yaml`
...
aios-launcher \
  ...
  [--show-deps]      # opcional: mostra dependências do prompt
...
│   ├── prompt_loader.py          # Load prompt from prompts.yaml
...
├── scripts/
│   └── universal/
│       └── aios-launcher.sh      # Bash wrapper (optional)
│
└── docs/
    └── mmos/
        ├── prompts.yaml              # Source of truth (59 prompts)
        └── logs/
            └── launcher-history.yaml  # Execution log
```
...
**Location:** `docs/mmos/prompts.yaml` (já existe)
...
|--------|------|---------|-------------|
| `--show-context` | Flag | False | Exibir contexto completo (não apenas resumo) |
| `--show-deps` | Flag | False | Exibir dependências do prompt |
...
📝 Dependencies (must complete first):
   ✅ analysis_behavioral_patterns (completed)
   ✅ analysis_recognition_patterns (completed)
   ✅ analysis_linguistic_forensics (completed)
...
📝 Execution logged to: docs/mmos/logs/launcher-history.yaml
```
...
|-----------------|----------|-----------|
| Mind não existe | Print error + list available minds in minds/ | 1 |
| Phase inválido | Print error + list valid phases | 1 |
| Prompt ID não existe | Print error + list available prompts for phase | 1 |
| docs/mmos/prompts.yaml missing | Print error + suggest fix | 2 |
| prompts.yaml malformed YAML | Print YAML parse error + line number | 2 |
| Context file missing (PRD, MIND_BRIEF) | Warn + continue (não é bloqueante) | 0 |
| Output directory não existe | Create directory + warn | 0 |
| Unable to write log | Warn + continue (não é bloqueante) | 0 |
...
```python
# Ordem de prioridade (melhor esforço, não bloqueante)
context_files = [
    "minds/{mind}/docs/MIND_BRIEF.md",     # Roadmap + viabilidade
    "minds/{mind}/docs/PRD.md",            # Objetivo + success criteria
    "minds/{mind}/sources/sources_master.yaml",  # Fontes disponíveis
]
```
...
**MIND_BRIEF.md:**
- Extract: Seção "2. Viabilidade (SCORECARD)" → APEX + ICP scores
- Extract: Seção "7. Roadmap" → checkpoints status (quantos completos)

**PRD.md:**
- Extract: Primeira linha after "## 1. Objetivo" → primary use case
...
Current Phase: {phase} ({completed}/{total} checkpoints completed)
```
...
- [ ] Load prompts.yaml (59 prompts)
- [ ] Implement CLI interface (Click)
- [ ] Implement prompt → agent mapping

**Day 1 Afternoon:**
- [ ] Implement context loading (MIND_BRIEF, PRD, sources)
- [ ] Implement context summarization
- [ ] Implement output path generation

**Day 2 Morning:**
- [ ] Implement execution logging (launcher-history.yaml)
- [ ] Implement formatted output display
- [ ] Add error handling

**Day 2 Afternoon:**
- [ ] Test with 3 real prompts (different phases)
- [ ] Fix bugs
...
**Testing:**
- [ ] Test all 6 phases (1 prompt each = 6 tests)
- [ ] Test error conditions (invalid mind, prompt, phase)
- [ ] Test with missing context files (graceful degradation)
- [ ] Measure time saved vs manual (baseline: 5min)

**Success Criteria:**
- ✅ 6/6 prompts executados com sucesso
- ✅ ≥30% redução tempo (5min → ≤3.5min)
- ✅ 100% logs registrados corretamente
- ✅ Zero modificações em minds/ (read-only)

### Phase 3: Iteration (Days 4-5)

**If successful (≥30% time saved):**
...
- Unit tests for core logic (prompt_loader, context_loader, logger)
...
4. **Context auto-fill no Claude Code** - Requer plugin/extensão
...
**Scenario:** Operator precisa executar prompt `analysis_mental_models` para Steve Jobs

**Manual Process (Baseline):**
1. Consultar documentação ou experiência para lembrar ID do prompt
...
5. Verificar manualmente se dependências foram completadas
...
4. Executar prompt
5. **Total: ~3 minutos** (40% redução)

**Success Criteria:**
- ✅ Tempo ≤3.5 minutos (≥30% redução)
- ✅ Operador não precisa abrir nenhum arquivo manualmente
- ✅ Dependencies automaticamente verificadas
...
|--------|-------------------|-------------------|----------|
| Tempo médio por prompt | 5 min | ≤3.5 min (30% redução) | TBD |
...
- [ ] "Contexto exibido foi útil?" (Yes/No/Partial)
- [ ] "Encontrou alguma fricção?" (descritivo)
- [ ] "Usaria launcher para todos os 59 prompts?" (Yes/No)

---

## 11. Risks & Mitigations

### R-LAUNCHER-1: prompts.yaml desatualizado (MÉDIO)
- **Problema:** docs/mmos/prompts.yaml fica desatualizado quando pipeline muda
- **Mitigação:**
  - Source of truth já versionado em git
...
- **Problema:** MIND_BRIEF.md ou PRD.md mudam de localização
- **Mitigação:**
  - Graceful degradation (warn + continue)
  - Não é bloqueante para MVP
  - Documentar estrutura ACS V3.0 como contrato

### R-LAUNCHER-3: Ganho <30% (ALTO)
- **Problema:** Launcher não economiza tempo suficiente (ROI negativo)
- **Mitigação:**
  - Validar cedo (Day 3)
  - Pivot rápido se não funcionar
  - Investigar onde está o bottleneck real (não assume é ativação)
...
1. **Investigate:** Onde está o bottleneck real?
...
2. **Pivot Options:**
   ...
   - Focar em outro pain point (G2: paralelização?)
...
📋 Full Context (--show-context enabled):
---
MIND_BRIEF Roadmap:
...
📝 Dependencies (--show-deps enabled):
   ✅ analysis_behavioral_patterns (completed 2025-10-03)
   ✅ analysis_recognition_patterns (completed 2025-10-03)
   ✅ analysis_linguistic_forensics (completed 2025-10-02)
...
---
💡 Tip: Use --dry-run to simulate without logging
```
...
**Structure:** Ver seção 2.3 para estrutura completa

**Para Consultar:** Abra `docs/mmos/prompts.yaml` para ver todos os prompts disponíveis com campos completos (agent, outputs, depends_on, parallelizable, etc.)
...
**Next Actions:**
1. ✅ Review spec com Alan Nicolas
