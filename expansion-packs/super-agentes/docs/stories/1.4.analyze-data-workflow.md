...
├── agents/
│   └── db-sage.md                           # Add *import, *analyze commands
├── workflows/
│   └── analyze-data-workflow.yaml           # NEW FILE
├── tasks/
│   ├── db-load-csv.md                       # CSV import with staging
│   └── db-seed.md                           # Idempotent seed data
└── docs/
    ├── epics/epic-db-sage-workflows.md
    └── stories/1.4.analyze-data-workflow.md
```

### Key Context from Research

**Industry Pattern (PostgreSQL 2024):**
- Staging → Validation → Merge pattern (never import directly to production)
...
❌ ERROR: unterminated CSV quoted field
   ...
   → Adjust COPY parameters: QUOTE '''', ESCAPE '\'

❌ ERROR: null value in column "id" violates not-null constraint
   → Check CSV for empty ID fields
   → Define NULL representation: WITH (NULL 'NULL')

❌ ERROR: duplicate key value violates unique constraint
   → Use ON CONFLICT in merge step
   → Or clean duplicates from staging before merge
```
...
✅ All 6 ACs implemented + **SQL queries completed** (no placeholders)

**Workflow:** `analyze-data-workflow.yaml` (307 lines - updated from 123)

**Analysis Features:**
- ✅ **Table Statistics:** Size breakdown (total/table/index), live/dead rows
- ✅ **Data Distribution:** Value counts, percentages, NULL analysis
- ✅ **Integrity Checks:** FK violations, missing PKs, dead row percentage
- ✅ **Recent Activity:** Recent records, time-based activity (30 days)
...
1. **CSV Import Path** - ✅ Fully functional via db-load-csv task
2. **Seed Data Path** - ✅ Fully functional via db-seed task
3. **Analysis Paths** - ✅ ALL COMPLETE:
...
2. `workflows/analyze-data-workflow.yaml` (updated: 123→307 lines)
...
This story demonstrates excellent implementation of PostgreSQL best practices and is ready for implementation. Key strengths:
- Staging → Validation → Merge pattern (industry standard for safe data imports)
- COPY command for bulk loads (10-100x faster than INSERT)
- Idempotent seed pattern (ON CONFLICT) for safe re-runs
- Comprehensive validation checks before merge
- Orphaned records detection (data integrity)
- Well-integrated with existing tasks (db-load-csv, db-seed)
...
- **Given**: User has CSV file or seed data
- **When**: User runs `*import` or `*analyze` workflow
- **Then**: Data imported safely via staging table OR analysis results displayed

**Test Coverage**: 11 scenarios documented
- 3 happy paths (CSV import small, CSV import large, seed data)
- 4 validation scenarios (NULL detection, duplicates, staging rollback, idempotency)
- 4 analysis scenarios (table stats, distribution, integrity checks, recent activity)

### Security Review

✓ **PASS** - Security design is excellent:
- Staging table pattern prevents direct production writes
- Validation checks before merge to production
- Idempotent seed pattern prevents double-application issues
...
- Batch processing for large files (>100MB)
- Staging table for bulk operations
- Performance tips documented (disable triggers, indexes during large loads)

### Non-Functional Requirements

- **Security**: ✓ PASS (staging pattern, validation before merge)
- **Performance**: ✓ PASS (COPY command, batch processing)
- **Reliability**: ✓ PASS (validation checks, idempotent seeds, orphaned records detection)
- **Maintainability**: ✓ PASS (clear patterns, comprehensive error handling)

### Recommendations

**Future Enhancements** (non-blocking):
- Consider data validation rules library for common patterns (email format, phone format, etc.)
- Consider progress indicator for large CSV imports (>1GB)
- Consider CSV template generator for common table structures

### Gate Status

Gate: PASS → docs/qa/gates/1.4-analyze-data-workflow.yml
