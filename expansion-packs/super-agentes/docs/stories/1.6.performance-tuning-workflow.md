...
- [ ] Integrate db-query-optimization task (AC: 3,4,5,6)
  - [ ] Map workflow to db-query-optimization.md for interactive optimization
  - [ ] Identify issues:
...
- [ ] Add common optimization patterns (AC: 3,4)
  - [ ] Pattern 1: Simple filter → single-column index
  - [ ] Pattern 2: Composite filter + sort → multi-column index
  - [ ] Pattern 3: Join → index on FK
...
- [ ] Update db-sage agent (AC: all)
  ...
  - [ ] Add `*optimize` command mapping to db-query-optimization.md
...
├── agents/
│   └── db-sage.md                           # Add *tune, *hotpaths, *optimize
├── workflows/
│   └── performance-tuning-workflow.yaml     # NEW FILE
├── tasks/
│   ├── db-analyze-hotpaths.md               # Find slow queries
│   ├── db-explain.md                        # EXPLAIN ANALYZE
│   ├── db-query-optimization.md             # Interactive optimization
│   └── db-rls-audit.md                      # RLS performance check
└── docs/
    ├── epics/epic-db-sage-workflows.md
    └── stories/1.6.performance-tuning-workflow.md
```
...
1. **Find Slow Queries** (pg_stat_statements)
   ...
   - Queries consuming most total time (calls × mean)
...
**Problem**: Estimated 10 rows, actually 10,000 → wrong join strategy
...
Changes Applied:
  ✓ Added index on posts(user_id, created_at DESC)
  ✓ Wrapped auth.uid() in RLS policy for caching
...
```
❌ ERROR: extension "pg_stat_statements" is not installed
   → Enable extension: CREATE EXTENSION IF NOT EXISTS pg_stat_statements;
   → Supabase: Should be enabled by default

❌ ERROR: permission denied for function pg_stat_statements
   → Use service_role key for admin operations
   → Or grant: GRANT EXECUTE ON FUNCTION pg_stat_statements TO user;

❌ ERROR: index "idx_name" already exists
   → Index already created
   → Or drop first: DROP INDEX CONCURRENTLY idx_name;

❌ ERROR: cannot explain a query in read-only transaction
   → Don't use ANALYZE in read-only mode
   → Use: EXPLAIN (without ANALYZE) for plan estimation only
```
...
- Well-integrated with 4 existing tasks (db-analyze-hotpaths, db-explain, db-query-optimization, db-rls-audit)

### Compliance Check

- Coding Standards: ✓ N/A (Draft story, no code yet)
- Project Structure: ✓ Follows expansion-packs structure
- Testing Strategy: ✓ Comprehensive test scenarios with measurement
...
🔥 **RLS Performance Optimization (AC 7)**:
...
✓ **PASS** - Security design includes performance:
...
- **Security**: ✓ PASS (RLS performance = security, prevents DoS)
- **Performance**: ✓ PASS (comprehensive analysis tooling)
- **Reliability**: ✓ PASS (performance targets, monitoring integration)
- **Maintainability**: ✓ PASS (optimization patterns library, clear examples)
...
Gate: PASS → docs/qa/gates/1.6-performance-tuning-workflow.yml
