...
interface MIU {
  // Identifiers
  fragment_id: string;              // "f_001"
  subject_id: string;               // "subject_nassim_taleb"

  // CORE: The MIU itself
  content: {
    verbatim: string;               // Exact text (preserves all punctuation)
    char_count: number;
    word_count: number;
    clause_count: number;           // May be > 1 if causal/temporal
  };

  // ATTRIBUTION: Who said/did this?
  attribution: {
    speaker: 'subject' | 'other' | 'group' | 'narrator';
    speaker_name: string | null;   // If "other", who specifically?
  };

  // SOURCE: Where did this come from?
  source: {
    document_id: string;
    document_type: string;          // "podcast_transcript", "email", "whatsapp"
    char_position: [number, number];
    timestamp: string | null;       // ISO 8601
    medium: string;                 // "spoken_transcribed", "written_digital"
    language: string;               // ISO 639-1 code
  };

  // MINIMAL CONTEXT: Just enough to interpret
  context: {
    sentence_before: string | null; // 1 sentence before
    sentence_after: string | null;  // 1 sentence after
    responding_to: string | null;   // If dialogue, what triggered this?
  };

  // STRUCTURE: Zero-inference linguistic facts
  structure: {
    // Words (tokenized, lowercased)
    words: string[];
    unique_words: string[];
    word_frequencies: Record<string, number>;

    // Grammar (NO interpretation)
    pronouns: string[];
    verbs: string[];
    verb_forms: string[];           // "present_simple", "past_participle"
    nouns: string[];
    adjectives: string[];
    adverbs: string[];
  ...
  // EXTRACTION METADATA
  extraction: {
    method: string;                 // "claude_sonnet_4_miu_v1"
