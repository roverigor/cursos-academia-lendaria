...
class TranscriptDownloader {
  ...
  async downloadGovernmentPDF(url, sourceId) {
    ...
    console.log(`   1. Download: curl -L -o /tmp/${sourceId}.pdf "${url}"`);
    console.log(`   2. Convert: pdftotext /tmp/${sourceId}.pdf /tmp/${sourceId}.txt`);
    ...
    return {
      ...
      requiresManual: true,
      instructions: [
        `curl -L -o /tmp/${sourceId}.pdf "${url}"`,
        `pdftotext /tmp/${sourceId}.pdf docs/minds/sam_altman/sources/manual/${sourceId}.txt`,
  ...
  async download(config) {
    const { sourceId, urls, sourceData } = config;
    ...
    for (const urlConfig of urls) {
      try {
        ...
        if (result.text && result.text.length > 100) {
          // Save immediately
          await this.saveTranscript(sourceId, result, sourceData);
          return result;
        }
      } catch (error) {
    ...
    throw new Error(`All download methods failed for ${sourceId}`);
  }

  async saveTranscript(sourceId, result, sourceData) {
